...
public class OperacionBancoServiceImpl implements OperacionBancoService {
	...
	public Mono<OperacionCuentaBanco> saveOperacionCuentaCuentaCredito(OperacionCuentaBanco operacion) {
		...
		log.info("datos cliente  --->> "+oper1.map(p -> {
			...
			System.out.println("Datos cuenta de banco : " + p.toString());
			System.out.println("Datos tipo cuenta de banco : " + p.getTipoProducto().toString());
			
			return p.getCodigoBanco();
		}));
		...
		return opCredito.defaultIfEmpty(new CuentaBanco()).flatMap(p->{
			...
			if(p.getCodigoBanco() == null) {
				...
				return oper1.flatMap(c1 -> {
					...
					} else if (c1.getTipoProducto().getIdTipo().equalsIgnoreCase("4")|| c1.getTipoProducto().getIdTipo().equalsIgnoreCase("5")
							|| c1.getTipoProducto().getIdTipo().equalsIgnoreCase("6")|| c1.getTipoProducto().getIdTipo().equalsIgnoreCase("7")
		...
		return oper1.flatMap(c1 -> {
			if (c1.getTipoProducto().getIdTipo().equalsIgnoreCase("1")) { //ahorro
				...
				if (c1.getSaldo() == 100) {
					...
					throw new RequestException("NO PUEDE REALIZAR RETIROS, MONTO MINIMO EN LA CUENTA S/.100");
				}
			}
